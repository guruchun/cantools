...
namespace FcpSims
{
    public partial class FormInverter : DockContent
    {
        // static members --------------------
        ...
        private void FormInverter_Load(object sender, EventArgs e)
        {
            try {
                // port configuration
                RefreshPorts();
        ...
        {
            modbusClient.IPAddress = ConfigInfo.GetSettings().DeviceProperties["Inv.ip"];
            modbusClient.Port = int.Parse(ConfigInfo.GetSettings().DeviceProperties["Inv.portNum"].ToString());
        ...
        {
            string inv_mapFilePath = ConfigInfo.GetSettings().DeviceProperties["Inv.mapFile"];
            string type = "Modbus";

            XmlSerializer serializer = new XmlSerializer(typeof(ModbusMap));
            TextReader reader = new StreamReader(Application.StartupPath + "/maps/" + inv_mapFilePath);
